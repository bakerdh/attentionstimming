...
% DHB 23/1/17, added eyetracking 11/7/22

close all;
W = what; E.exptpath = strcat(W.path,'/'); clear W;
useVP = 1;
trackermode = 1;              % 0 means no eye tracking, 1 means tracking
...
ST.basecontrast = 0.5;
ST.inccontrast = 0.66;  % small contrast increment to one half of grating
...
try                 % start the 'try/catch' loop
    ...
    while E.currentblock < E.blockstorun
        ...
        if E.stimorder==0
            if rem(E.currentblock,2)
                Screen('DrawText', w, 'Please keep still and do not stim during this block',ST.width/2-250,ST.height/2,0);
                R.isstimming(E.currentblock) = 0;
            else
                Screen('DrawText', w, 'Please stim throughout this block',ST.width/2-250,ST.height/2,0);
                R.isstimming(E.currentblock) = 1;
            end
        end
        if E.stimorder==1
            if rem(E.currentblock,2)
        ...
        if trackermode == 1              % initialize eyelink
            ...
            Eyelink('OpenFile', 'temp.edf');       % open up the file to save eyetracker data to
            Eyelink("command", "calibration_area_proportion = .5, .5")
            Eyelink("command", "validation_area_proportion = .5, .5")
        ...
        if useVP
            for n = 1:3                         % 3 trigger pulses to indicate start of block
                Datapixx('SetDoutValues', transformindex(1+R.isstimming(E.currentblock)));
                Datapixx('RegWrRd');
                WaitSecs(0.05);
        ...
        if trackermode
            Eyelink('Message',strcat('Stim mode:',num2str(1+R.isstimming(E.currentblock))));
        end
        ...
        while trial < E.ntrialsperblock
            ...
            if trackermode
                Eyelink('Message',strcat('Trial',num2str(100*cuetype + cueside),'_onset'));
            end
            ...
            if trackermode == 1
                Eyelink('Message',strcat('Trial',num2str(100*cuetype + cueside),'_offset'));
            end
            ...
            while n < targetframes
                ...
                if trackermode
                    if n==1
                        Eyelink('Message',strcat('Trial',num2str(targetside*10+incside),'_onset'));
                    end
                    if n==(targetframes-1)
                        Eyelink('Message',strcat('Trial',num2str(targetside*10+incside),'_offset'));
                    end
                end
        ...
        if ~breakcode
            ...
            if trackermode
                ...
                Eyelink('CloseFile');
                copyfile('temp.edf',strcat('Results/',E.subj,'eyetrackR',num2str(E.currentblock),'.edf'));
